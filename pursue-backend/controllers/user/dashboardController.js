...
exports.getUserDashboardAnalytics = async (req, res) => {
  try {
    ...
    if (spaceIds.length) bookingFilter.spaceId = { $in: spaceIds };
    ...
    const dailyUsage = {};
    bookings.forEach(b => {
      const day = new Date(b.startDate).toLocaleDateString("en-IN", { weekday: "short" });
    ...
    res.json({
      ...
      data: {
        totalSpent,
        hoursBooked,
        spacePie: Object.entries(spaceDistribution).map(([name, value]) => ({ name, value })),
        branchPie: Object.entries(branchDistribution).map(([name, value]) => ({ name, value })),
        usageTrend: Object.entries(dailyUsage).map(([name, hours]) => ({ name, hours })),
      },
    });
  } catch (error) {
